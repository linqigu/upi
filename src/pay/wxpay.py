...
from src.config import request_enum
from src.util.tool import is_url, get_timestamp, check_float, trans_dict_to_xml, trans_xml_to_dict, UpiException
from src.util.log import logger, set_logger
...
class wxPay(object):
    def __init__(self,
                 ...
                 sub_app_id=None,
                 sub_mch_id=None,
                 mch_type=type_enum.MERCHANT_GENERAL.value,
                 api_time_out=request_enum.API_TIME_OUT.value,
                 pay_host=type_enum.PAY_HOST_MASTER.value,
                 logger_echo=False
                 ):
        """
    ...
    def js_pay(self, order_no, fee, body, openid, goods_tag=None, attach=None):
        ...
        :return:
        """
        return self.__package_pay_data(order_no, fee, body, goods_tag, attach, apply_type=type_enum.JS_PAY.value, openid=openid)
    ...
    def app_pay(self, order_no, fee, body, goods_tag=None, attach=None):
        ...
        :return:
        """
        return self.__package_pay_data(order_no, fee, body, goods_tag, attach, apply_type=type_enum.APP_PAY.value)
    ...
    def native_pay(self, order_no, fee, body, goods_tag=None, attach=None):
        ...
        :return:
        """
        return self.__package_pay_data(order_no, fee, body, goods_tag, attach, apply_type=type_enum.NATIVE_PAY.value)
    ...
    def h5_pay(self, order_no, fee, body, goods_tag=None, attach=None):
       ...
       :return:
       """
        return self.__package_pay_data(order_no, fee, body, goods_tag, attach, apply_type=type_enum.H5_PAY.value)
    ...
    def __package_pay_data(self, order_no, fee, body, goods_tag, attach, apply_type, openid=None):
        ...
        if self.mch_type == type_enum.MERCHANT_GENERAL.value:
            # 为普通商户
            pass
        elif self.mch_type == type_enum.MERCHANT_SERVICE.value:
            # 服务商
            pay_detail["sub_app_id"] = self.sub_app_id
            pay_detail["sub_mch_id"] = self.sub_mch_id
        elif self.mch_type == type_enum.MERCHANT_BANK.value:
            # 银行商户
            raise UpiException("当前sdk不支付银行商户支付")
        res = {'pay_data': None}
        self.request_url = "".join([self.pay_host, type_enum.PAY_URL_UNIFIED_ORDER.value])
        if apply_type in [type_enum.JS_PAY.value, type_enum.MINI_PAY.value]:
            # 公众号支持与小程序支付一致
            pay_detail['trade_type'] = type_enum.TRADE_TYPE_JS.value
        ...
        if apply_type == type_enum.NATIVE_PAY.value:
            pay_detail['trade_type'] = type_enum.TRADE_TYPE_NATIVE.value
            pay_detail["sign"] = self.__sign(pay_detail)
            r = self.__send_request(pay_detail)
            res.update({'pay_data': r['code_url']})
        if apply_type == type_enum.H5_PAY.value:
            pay_detail['trade_type'] = type_enum.TRADE_TYPE_H5.value
            pay_detail["sign"] = self.__sign(pay_detail)
            r = self.__send_request(pay_detail)
            res.update({'pay_data': r['mweb_url']})
        if apply_type == type_enum.APP_PAY.value:
            pay_detail['trade_type'] = type_enum.TRADE_TYPE_APP.value
            pay_detail["sign"] = self.__sign(pay_detail)
            r = self.__js_package(self.__send_request(pay_detail))
            res.update({'pay_data': r})
        return res
    ...
    def __check_common(self):
        ...
        :return:
        """
        if self.mch_type not in [type_enum.MERCHANT_GENERAL.value, type_enum.MERCHANT_SERVICE.value,
                                 type_enum.MERCHANT_BANK.value]:
            raise UpiException("商户号类型输入错误,您输入的为:{}".format(self.mch_type))
        if self.mch_type == type_enum.MERCHANT_SERVICE.value:
        ...
        if self.mch_type == type_enum.MERCHANT_BANK.value:
            raise UpiException("当前sdk不支付银行商户支付")
        # 检测回调地址是否合法
